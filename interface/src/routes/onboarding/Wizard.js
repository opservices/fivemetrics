...
export default class Wizard extends Component {
  ...
  renderSteps() {
      ...
      return this.props.steps.map((s, i)=> (
        <ButtonGroup key={i}>
          <Button className='progress-step-button' value={i} active={(this.state.compState == i) ? true : false} outlined style={{marginBottom: 5, cursor: 'auto'}} bsStyle='purple'>{i+1}</Button>
        </ButtonGroup>
        ));
      ...
      <div className="multi-step" onKeyDown={(evt) => {this.handleKeyDown(evt)}}>
          {
             this.props.showSteps
                  ? <ButtonToolbar className='progress-step'>
                    {this.renderSteps()}
                    </ButtonToolbar>
...
Wizard.defaultProps = {
  ...
  startAtStep: 0,
  nextButtonText: "Next",
  nextButtonCls: "btn btn-prev btn-lg pull-right btn-lightgreen btn-outlined btn-default",
  backButtonText: "Previous",
  backButtonCls: "btn btn-next btn-lg pull-left btn-lightgreen btn-outlined btn-default",
